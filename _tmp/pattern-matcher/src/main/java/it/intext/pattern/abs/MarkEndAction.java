...
public class MarkEndAction implements Action{

	private List<String> addElements = new ArrayList<String>();
	static final Pattern LAYERPATTERN = Pattern.compile("^([A-z0-9]+)#([^#]+?)$"); 
	static final Logger logger = LoggerFactory.getLogger(MarkEndAction.class);
	...
	public boolean executeAction(Result result, QueryMatchParser qm) {
		...
		for(ResultReport repo : result.getReports())
		{
			...
			tg.setPositions(new int[]{repo.getMatchEndPosition()});
				...
				if (matcher.matches())
				{
					...
					if (text.contains("."))
					{
						try{
							...
							logger.debug("End marked: {} {}.", term, tg.getPositions());
							continue;
						}catch(Exception e){}
					}
