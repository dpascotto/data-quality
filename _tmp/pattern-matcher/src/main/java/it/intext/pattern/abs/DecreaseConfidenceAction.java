...
public class DecreaseConfidenceAction implements Action{

	private List<String> targets = new ArrayList<String>();
	static final Pattern LAYERPATTERN = Pattern.compile("^([A-z0-9]+)#([^#]+?)$"); 
	static final Logger logger = LoggerFactory.getLogger(DecreaseConfidenceAction.class);
	...
	public TYPE getType() {
		return TYPE.DECREASE_CONFIDENCE;
	}
	...
	public boolean executeAction(Result result, QueryMatchParser qm) {
		...
		for(String tm : this.targets)
		{
			...
			if (matcher.matches())
			{
				...
				idx.addLayer(layer);
				try{
				done |= idx.getLayer(layer).decreaseConfidence(text, tg.getPositions());
				}catch(Exception e)
				{
					logger.warn("Error decreasing confidence for : {}",tm);
				}
			}
			else
